        ...
        if !self.includes_entities {
            ...
            if self.is_current_entity {
                ...
                if self.is_world_limited {
                    self.add_entities(&mut list, source.world().as_ref(), &predicate)
                        .await;
                } else {
                    let worlds = source.server().worlds.load_full();
                    for world in worlds.iter() {
                        self.add_entities(&mut list, world, &predicate).await;
                    }
                }
...
pub enum EntitySelectorPredicate {
    ...
    /// A predicate to check a vanilla or data-pack-defined entity type tag.
    EntityTypeTag(Identifier, bool),
                    ...
                    let is_tagged =
                        base_entity
                            ...
                            .is_some_and(|server| {
                                server
                                    .data_pack_manager
                                    .entity_type_is_tagged(expected_tag, base_entity.entity_type)
                            });
                    is_tagged ^ invert
                }
